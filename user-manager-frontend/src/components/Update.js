...
class Update extends React.Component {
    ...
    updateUser(e) {
        ...
        axios.put('http://localhost:8080/user/update/'+userId, user).then(function(response) {
            e.setState({
...
render() {
   if(this.state.messageFromServer == ''){
    ...
    return (
     <div>
         <Button bsStyle="warning" bsSize="small" onClick={this.openModal}><span className="glyphicon glyphicon-edit"></span></Button>
         <Modal isOpen={this.state.modalIsOpen} onAfterOpen={this.afterOpenModal} onRequestClose={this.closeModal} contentLabel="Update User" className="Modal">
             <div className='button-center'>
                    <h3>{this.state.messageFromServer}</h3>
                 <Link to={{pathname: '/', state:this.state}}style={{ textDecoration: 'none' }}>
